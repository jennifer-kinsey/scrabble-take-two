...
describe('numbers_to_words#isNumber?') do
  let(:number) { NumbersToWords.new }

  it("returns true if the number is an integer") do
    expect(number.isNumber?("1")).to(eq(true))
  end
  it("returns false if the number is not an integer") do
    expect(number.isNumber?("f")).to(eq(false))
  end
end

describe('numbers_to_words#transform_number') do
  let(:number) { NumbersToWords.new }
